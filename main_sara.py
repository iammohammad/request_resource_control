...
miot_arrival_rate = 0
arrival_rates = [20]  # [15,10,7,5,3,1] #[100,60,40,30,25,20,15,10,7,5,3,1] #[20,40,10,
...
# pct_inst_embb_size = 10 #percentage of instanced slices of type embb
...
# 7actions:
actions = [(1, 1, 1), (0.75, 1, 1), (1, 1, 0.75), (0.75, 1, 0.75), (0.75, 1, 0.5), (0.5, 1, 0.5),
           (0.5, 1, 0.25)]  # list of tuples
...
def update_resources(substrate, nslr, kill):
    ...
    # the nodes of the reduced graph of the accepted nslr are traversed = the nodes of the reduced graph of the accepted nslr are used
...
def resource_allocation(cn):  # cn=controller
    ...
    return accepted_reqs_inthiswindow, step_profit, step_node_profit, step_link_profit, step_embb_profit, step_urllc_profit, step_miot_profit, step_total_utl, step_node_utl, step_links_bw_utl, step_edge_cpu_utl, step_central_cpu_utl
...
def func_twindow(c, evt):
    ...
    # the list of reqs is filtered depending on the action
    granted_req_list, remaining_req_list = prioritizer(sim.window_req_list, a)
    sim.granted_req_list = granted_req_list
    # the list is sent to the Resource Allocation module
    accepted_reqs_inthiswindow, step_profit, step_node_profit, step_link_profit, step_embb_profit, step_urllc_profit, step_miot_profit, step_total_utl, step_node_utl, step_links_bw_utl, step_edge_cpu_utl, step_central_cpu_utl = resource_allocation(
        c)
    ...
    evt = sim.create_event(tipo="twindow_end", inicio=sim.horario + twindow_length,
                           extra={"first_state": False, "end_state": end_state, "current_state": s, "action": a},
                           f=func_twindow)
    sim.add_event(evt)
    sim.window_req_list = [[], [], []]
...
def prepare_sim(s):
    evt = s.create_event(tipo="arrival", inicio=s.horario + get_interarrival_time(embb_arrival_rate),
                         extra={"service_type": "embb", "arrival_rate": embb_arrival_rate}, f=func_arrival)
    s.add_event(evt)
    evt = s.create_event(tipo="arrival", inicio=s.horario + get_interarrival_time(urllc_arrival_rate),
                         extra={"service_type": "urllc", "arrival_rate": urllc_arrival_rate}, f=func_arrival)
    s.add_event(evt)
    evt = s.create_event(tipo="arrival", inicio=s.horario + get_interarrival_time(miot_arrival_rate),
                         extra={"service_type": "miot", "arrival_rate": miot_arrival_rate}, f=func_arrival)
    s.add_event(evt)

    evt = s.create_event(tipo="twindow_end", inicio=s.horario + twindow_length,
                         extra={"first_state": True, "end_state": False}, f=func_twindow)
    s.add_event(evt)
...
def main():
    ...
    for m in arrival_rates:
        ...
        for i in range(repetitions):
            ...
            agente = ql.Qagent(0.9, 0.9, 0.9, episodes, n_states,
            ...
            for j in range(episodes):
                ...
                acpt_rate_embb_rep[j].append(
                    controller.simulation.embb_accepted_reqs / controller.simulation.total_embb_reqs)
                acpt_rate_urllc_rep[j].append(
                    controller.simulation.urllc_accepted_reqs / controller.simulation.total_urllc_reqs)
                acpt_rate_miot_rep[j].append(
                    controller.simulation.miot_accepted_reqs / controller.simulation.total_miot_reqs)
            ...
            f = open(my_path + "/outputs/" + "sara_" + str(m) + "_64BA_3de5sta_7act_400epi_prioritizerv6.txt", "w+")
